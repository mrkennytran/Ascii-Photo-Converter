...
namespace ASCII_Generator {
    class BitmapAscii {
        ...
        #region Update WIP - must initialize kernal by find the average of given kernel width and height
        private string Asccitize2(BitmapMaker imgSource, int kernelWidth, int kernelHeight) {
            //Contains the ascii text version of the picture
            string finalAsciiString = String.Empty;
            double normalizedVal;
            string asciiCharacter;
            ...
            for (int y = 0; y < imgSource.Height; y += kernelHeight) {
                for (int x = 0; x < imgSource.Width; x += kernelWidth) {
                    ...
                    //Kernelization
                    for (int pY = kY; pY < kY + (kernelHeight - 1); pY++) {
                        for (int pX = kX; pX < kX + (kernelWidth - 1); pX++) {
                            //Get current pixel color
                            Color pixelColor = imgSource.GetPixelColor(x, y);

                            //Get avg pixel
                            double avgPixel = AveragePixel2(pixelColor);

                            //Convert to greyscale 
                            Color grey = Color.FromArgb(pixelColor.A, (byte)avgPixel, (byte)avgPixel, (byte)avgPixel);
        ...
        private double AveragePixel2(Color rgb) {
            //Luminosity algorithm (the perceived brightness of a colour)
            //Green is weighted more for perception            
            return ((.21 * rgb.R) + (.72 * rgb.G) + (.07 * rgb.B));
        }//end method

        private double AverageColor2(List<Color> pixelColors) {
            double normalizedKernelValue = 0.0;
            //Returns normalized value from the avg pixel 
            for (int index = 0; index < pixelColors.Count; index++) {
                normalizedKernelValue = (AveragePixel(pixelColors[index])) / 255;
            }//end 
